...
def generate_all_kline_images():
    ...
    print(f"\n共生成 {count} 张K线图，成功股票 {success_stocks}，失败/跳过股票 {failed_stocks}")
    return count
...
def update_db_image_urls():
    ...
    cursor.execute("""
        ...
        FROM questions
        WHERE level_id IN ('K1', 'K2', 'K3', 'K4', 'P1', 'P2', 'P3')
    """)
    for q_id, title, level_id in cursor.fetchall():
        # 从题目中提取股票代码
        match = re.search(r'（(\d+\.\w+)）', title)
        if match:
            code = match.group(1)
            if level_id in ('P1', 'P2', 'P3'):
...
if __name__ == "__main__":
    ...
    try:
        print("生成 K 线图...")
        generated_count = generate_all_kline_images()
        print("\n更新数据库图片链接...")
        updated_count = update_db_image_urls()
        if generated_count == 0:
            print("⚠️  未生成任何 K 线 PNG，后续应清理数据库中的失效 image_url")
            sys.exit(2)
        if updated_count == 0:
            print("⚠️  没有更新任何题目的 image_url")
            sys.exit(3)
    except Exception as e:
        print(f"❌ 生成 K 线图失败: {e}")
        traceback.print_exc()
